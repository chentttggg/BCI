...
class CompleteSensorGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, mock_mode=False, default_port=None):
        ...
        # Recordings go into the experiment project's Data/ folder.
        self.default_edf_output_dir = Path(__file__).resolve().parents[4] / "Data"
    ...
    def setup_tab_control(self):
        ...
        self.txt_edf_filename.setText(str(self.default_edf_output_dir))
    ...
    def start_recording(self):
        ...
        if not filename:
            filename = str(self.default_edf_output_dir)

        # SDK's start_edf_recording treats the second argument as an output
        # directory. Ensure it exists before starting the recorder.
    ...
    def on_eeg_received(self, packets):
        ...
        converted_samples = []
        for p in packets:
            if hasattr(p, "to_microvolts"):
                try:
                    converted_samples.append(p.to_microvolts(current_gain))
                except Exception as e:
                    print(f"EEG conversion error: {e}")
                    converted_samples.append(getattr(p, "channel_data", getattr(p, "adc_values", [0.0] * 8)))
            elif hasattr(p, "channel_data"):
                converted_samples.append(p.channel_data)
            elif hasattr(p, "adc_values"):
                converted_samples.append(p.adc_values)
            else:
                converted_samples.append([0.0] * 8)
